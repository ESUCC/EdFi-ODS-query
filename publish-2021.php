...
// publish-2021.php
//
//   SRS main ADVISER publishing process for the 2020-2021 ODS. This runs in a
...
printf("\n%s: INFO: publish-2021 starting up\n", date("Y-m-d H:i:s T"));
...
function createSPEDJson($id_county, $id_district, $edfiRec)
{
  ...
  if ($edfiRec->SpecialEducationSetting == 10)
  {
    // As of 2020-2021, we report new setting codes for public school students
    // instead of numeric values for hours per week of school and special education
    // time. For public students grades 1-12, the setting code based on SPED time
    // is stored in the edfi2 table, SpecialEducationHoursPerWeek field.
    $edfiRec->SpecialEducationSetting = $edfiRec->SpecialEducationHoursPerWeek;
  }
  $json = '{
...
{
  global $logFile;
  $logFile = fopen("logs/publish-2021.log", "a");
  if (!$logFile)
  {
...
function testAdviserValues($edfiRec)
{
  ...
  if ( $beginTimestamp < strtotime('2020-07-01') ||
    $beginTimestamp > strtotime('2021-06-30'))
  {
  ...
  if (!empty($edfiRec->EndDate))
  {
    ...
    if ( $endTimestamp < strtotime('2020-07-01') ||
      $endTimestamp > strtotime('2021-06-30') ||
